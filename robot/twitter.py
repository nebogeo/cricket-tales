...
import xmlrpclib, sys, os, time, tweepy, time, string_maker, random
...
def tweet_api(secrets,i):
    auth = tweepy.OAuthHandler(secrets["tweetauth0-"+str(i)],
                               secrets["tweetauth1-"+str(i)])
    ...
    auth.set_access_token(secrets["tweetauth2-"+str(i)],
                          secrets["tweetauth3-"+str(i)])
    return tweepy.API(auth)

def tweet(dir,api):
    if os.path.isfile(dir+"/msg"):
        try: msg = read_msg(dir)
        except: return
        #print(msg)
        os.remove(dir+"/msg")
        txt = build_msg(msg)
        # don't want to flood twitter (127 max daily msgs)
...
tweetdirs = [[location+"/TreeSpirit",tweet_api(secrets,0)],
             [location+"/ShrubSpirit",tweet_api(secrets,1)],
             [location+"/CoverSpirit",tweet_api(secrets,2)],]
